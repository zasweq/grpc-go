...
func newBuilderWithConfigForTesting(config []byte) (resolver.Builder, error) {
	return &xdsResolverBuilder{
		newXDSClient: func(name string, mr stats.MetricsRecorder) (xdsclient.XDSClient, func(), error) { // what to do with this wrt metrics recorder?
			return xdsclient.NewForTesting(xdsclient.OptionsForTesting{Name: name, Contents: config, MetricsRecorder: mr})
		},
	}, nil
}
...
func newBuilderWithClientForTesting(client xdsclient.XDSClient) (resolver.Builder, error) {
	return &xdsResolverBuilder{
		newXDSClient: func(string, stats.MetricsRecorder) (xdsclient.XDSClient, func(), error) { // or this? Do I just ignore this? Any problems that causes...?
...
func init() {
	...
	rinternal.NewXDSClient = xdsclient.New // this will now take a metrics recorder in its constructor, fix tests that use this and the mocking of this?
}

type xdsResolverBuilder struct {
	newXDSClient func(string, stats.MetricsRecorder) (xdsclient.XDSClient, func(), error) // this is set numerous places, where is this read and what happens to second argument?
}
...
func (b *xdsResolverBuilder) Build(target resolver.Target, cc resolver.ClientConn, opts resolver.BuildOptions) (_ resolver.Resolver, retErr error) {
	...
	newXDSClient := rinternal.NewXDSClient.(func(string, stats.MetricsRecorder) (xdsclient.XDSClient, func(), error))
