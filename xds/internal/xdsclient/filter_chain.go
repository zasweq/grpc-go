...
type FilterChain struct {
	...
	//
	// Only one of RouteConfigName adn InlineRouteConfig is set.
...
func filterChainFromProto(fc *v3listenerpb.FilterChain) (*FilterChain, error) {
	// Pull http Filters from LDS persist here
	filterChain, err := processNetworkFilters(fc.GetFilters())
	// However, those http Filters can also be overriden by RDS, which will
	// be also determined by these network filters (HCM Sits at end of network filter chain,
	// which determines RDS.
	// lis sees these persisted HTTP Filters + (Inline || RouteName), starts a watch
	// "Routed the same as client side"
	if err != nil {
		return nil, err
	}
