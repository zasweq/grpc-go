...
type config struct {
	...
	config *rpb.RBAC
	// rpb.RBACPerRoute // Contains a pointer to RBAC only, this needs to be looked into
}
...
func parseConfig(cfg proto.Message) (httpfilter.FilterConfig, error) { // what's flow of how these are called again - look at xds.go
	if cfg == nil {
		...
		return nil, fmt.Errorf("rbax: nil configuration message provided")
	}
...
func (i *interceptor) AllowRPC(ctx context.Context) error {
	// Is there any other functionality that needs to be added here? (i.e. scoped to the functionality of the xDS Configured RBAC Engine?)

	// ":method can be hard-coded to POST if unavailable" - A41
	ctx = metadata.AppendToOutgoingContext(ctx, ":method", "POST")
	return i.chainEngine.IsAuthorized(ctx)
}

// TODO ONCE GET BACK: FINISH CLEANUP PLUS THE RBAC PER ROUTE.RBAC THING