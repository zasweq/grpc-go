...
// Problem: how to get a ref to plugin option through internal only api?

// csm observability: OTel API 1:1(), alongside configuring Dial Option/ServerOption

// gets target (put Doug's comment here) *how it will call into csm plugin option once it receives cc/target*
// server side easily just call plugin option unconditionally if set

// ok..client side that seems pretty easy actually use a dial option that is
// just grpc.WithDefaultCallOption and the call option it adds just adds
// something to the context for all calls (if we should add labels (maybe based
// on target which is final after processing?)). the stats handler or
// interceptor would see it



// but how does this instantiate the csmpo?
// can't add it to metrics api
// internal only *through internal/* function on it that takes a plugin option
// and grabs ref...internal only type
// exported interface in internal/


type serverStatsHandler struct {
	o Options

	// global option for all servers with this configured, simple
