...
// to write my own comparisons on got vs. want - declare my own wantStruct types here...?

// i.e. spanContextInformation or something
// what I did for metrics - compareX(gotX, gotY) <- call this from top level Equal method on the trace/span information data type...
// did this for rows logic and also agg data type...
// trace.SpanContext comparison only compare trace id, span id, and traceoptions is sampled int seems like an important piece of information...?
...
type MessageEvent struct {
    Time                 time.Time can't really verify this...
...
type Link struct {
    TraceID    TraceID
    SpanID     SpanID
    Type       LinkType // should be link type child for remote parent as written (not remote parent...)
    Attributes map[string]interface{} // i think just ignore we don't set these anyway
}
*/
...
// can I just persist this span data? oh no I want an equal method on this
// struct type...for Equality with cmp.Diff

// stats handler end + final attempt...

// dial option to overwrite traces and logging,
// plumb that in the client conn passed to client library
// start client conn

/*
// read/build off reading from the wire, probably persisted, but used to fill out remote parent
// yes it is persisted - s.spanContext = parent, persisted and used to fill out parent, so yeah sure why not test
type SpanContext struct { // how is this related to links?
    TraceID      TraceID
    SpanID       SpanID
...
type Link struct {
	// TraceID of the span wrt the trace that is linked...
    TraceID    TraceID
	// SpanID of the span that is linked...
    SpanID     SpanID
    Type       LinkType
    Attributes map[string]interface{}
}
...
type SpanData struct {
    SpanContext
    ParentSpanID  SpanID
    SpanKind      int // server, client or unspecified
    Name          string
    StartTime     time.Time // dont need this no way to reliably test...server latency? that's for metrics don't need start time or end time...
    EndTime       time.Time
    Attributes    map[string]interface{} // hardcoded attributes I just decided to leave in there because why not? bool attribute client and fail fast
    Annotations   []Annotation  // idk what this is for seems not to be relevant
    MessageEvents []MessageEvent // probably want a compare message events too - you need to get this to a point where it fails when I declare message events with actual IDs
    Status // unnamed? In a Go structure, you are allowed to create anonymous fields. Anonymous fields are those fields which do not contain any name you just simply mention the type of the fields and Go will automatically use the type as the name of the field.
    Links                    []Link // a pointer from the current span to another span
    HasRemoteParent          bool
    DroppedAttributeCount    int // ignore dropped
