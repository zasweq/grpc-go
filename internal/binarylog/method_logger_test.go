...
func (s) TestTruncateMetadataNotTruncated(t *testing.T) {
	testCases := []struct {
		...
		{
			ml: NewMethodLoggerImp(maxUInt, maxUInt),
			mpPb: &pb.Metadata{
				Entry: []*pb.MetadataEntry{
					{Key: "", Value: []byte{1}},
				},
		...
		{
			ml: NewMethodLoggerImp(2, maxUInt),
			mpPb: &pb.Metadata{
				Entry: []*pb.MetadataEntry{
					{Key: "", Value: []byte{1}},
				},
		...
		{
			ml: NewMethodLoggerImp(1, maxUInt),
			mpPb: &pb.Metadata{
				Entry: []*pb.MetadataEntry{
					{Key: "", Value: nil},
				},
...
func (s) TestTruncateMessageNotTruncated(t *testing.T) {
	testCases := []struct {
		...
		{
			ml: NewMethodLoggerImp(maxUInt, 3),
			msgPb: &pb.Message{
				Data: []byte{1, 1},
			},
		},
		{
			ml: NewMethodLoggerImp(maxUInt, 2),
			msgPb: &pb.Message{
				Data: []byte{1, 1},
			},
		},
