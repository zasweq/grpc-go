...
func SetupManagementServerAndResolver(t *testing.T) (*ManagementServer, string, []byte, resolver.Builder) {
	// Start an xDS management server.
	xdsServer := StartManagementServer(t, ManagementServerOptions{AllowResourceSubset: true})

	// Create bootstrap configuration pointing to the above management server.
	nodeID := uuid.New().String()
	bc := DefaultBootstrapContents(t, nodeID, xdsServer.Address)
